...
from .thread import RunnerThread
...
class DiagnosticsInfoI(vectorized.Filter):
  ...
  def _after_reset(self, observation):
    tf.logging.info('Resetting environment')
    self._episode_reward = 0
    self._episode_length = 0
    self._all_rewards = []
    return observation
  ...
  def _after_step(self, observation, reward, done, info):
    ...
    if done:
      tf.logging.info('Episode terminating: episode_reward=%s episode_length=%s', self._episode_reward, self._episode_length)
